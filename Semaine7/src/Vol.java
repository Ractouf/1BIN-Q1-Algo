...
public class Vol {
	...
	public Coordonnees lieuLePlusEloigne(){
		...
		int x = 0;
		for (int i = 0; i < tableCoordonnees.length; i++) {
			if (tableCoordonnees[0].distance(tableCoordonnees[i]) > max) {
				max = tableCoordonnees[0].distance(tableCoordonnees[i]);
				x = i;
			}
		}
	...
	public boolean cibleAtteinte(Coordonnees cible){
		...
		for (Coordonnees tableCoordonnee : tableCoordonnees) {
			if (cible.equals(tableCoordonnee))
				return true;
		}
		return false;
	}
	...
	public int nombreCiblesAtteintes(Coordonnees[] cibles){
		int sommeCiblesAtteintes = 0;
		if(cibles==null)
			throw new IllegalArgumentException();
		for (Coordonnees cible : cibles) {
			if (cibleAtteinte(cible))
				sommeCiblesAtteintes += 1;
		}
		return sommeCiblesAtteintes;
	}
	...
	public double distance() {
		double sommeCoordonnees = 0;
		for (int i = 0; i < tableCoordonnees.length - 1; i++) {
			sommeCoordonnees += tableCoordonnees[i].distance(tableCoordonnees[i + 1]);
		}
		return sommeCoordonnees;
	}
	...
	public Coordonnees lieuLePlusAuSud() {
		// Cette methode est proposee en exercice supplementaire!
